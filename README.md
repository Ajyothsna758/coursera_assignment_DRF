...
1. Dealt with 3 types of persons (Managers, Customers and Delivery crew)
   - user registration and authentication
      - `http://127.0.0.1:8000/api/users/`
   - assigned users to groups or remove users from groups
      - `http://127.0.0.1:8000/api/users/{userId}/groups`
2. Assigned work to users
3. If manager:
...
5. Created APIs that will allow customers to add menu items to their cart and place an order. But here's the catch, the cart must be emptied when the order is successfully created
6. Added an API endpoints to flush the cart at any point. One customer should only be able to have one cart at a time, and one cart should be able to contain multiple menu items.
- The Delivery process:
1. First, created API endpoints for the managers to browse the orders and assign them to a delivery person. 
...
5.  Throttling Process:
    - Implemented some throttling for the APIs you built, limit it to five API calls per minute. 

## Tools:
- used VS Code for writing the code, 
- Used Django with Django REST framework or DRF, 
- For testing and debugging used insomnia,

# API Endpoints Overview
## User Groups:
- Created 3 types of users, __managers, customers, and the delivery crew__.
- Started with the user registration and authentication process that each user will use, then created API endpoints that can be used to assign users to a group like manager or a delivery person.
